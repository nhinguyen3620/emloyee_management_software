...
BEGIN
    if NOT EXISTS (SELECT * FROM Project WHERE NEW.WorkON = ProjID) AND NEW.WorkON IS NOT NULL then
        RAISE EXCEPTION 'Value must exist in Project table or be Null. Type in a different value';
    END IF;
